...
- **P2P Akışı:** Düğümler bir blok için `FinalityCert` aldığında, eğer imza detaylarını doğrulamak isterlerse `ChainActor` üzerinden asenkron olarak `GetQcBlob` isteği gönderirler.
- **Depolama:** Bu bloblar diskte `QC:{Height}` anahtarı altında saklanır. Gerektiğinde `ChainHandle` üzerinden sorgulanarak ağda paylaşılabilir.
