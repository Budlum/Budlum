...
mod hardening_tests {
    ...
    fn test_merkle_state_root_determinism() {
        ...
        assert_eq!(root1, root2, "Merkle root must be deterministic regardless of insertion order");
        assert_ne!(root1, "0".repeat(64), "Root should not be empty");
        
        state1.add_balance("alice", 1);
        assert_ne!(root1, state1.calculate_state_root(), "Root must change when balance changes");
    }
    ...
    fn test_metrics_encoding_format() {
        ...
        assert!(encoded.contains("budlum_chain_height 1234"), "Encoded metrics should contain height");
        assert!(encoded.contains("budlum_peer_count 5"), "Encoded metrics should contain peer count");
        assert!(encoded.contains("# HELP budlum_chain_height"), "Encoded metrics should contain HELP metadata");
    }
