...
mod tests {
    ...
    async fn test_rpc_chain_info() {
        let (server, _) = setup();
        assert_eq!(server.chain_id().unwrap(), 1337);
    ...
    async fn test_rpc_block_methods() {
        let (server, bc) = setup();
        assert_eq!(server.block_number().unwrap(), 1);
        
        let genesis_hash = bc.lock().unwrap().chain[0].hash.clone();
        let block_by_hash = server.get_block_by_hash(genesis_hash.clone()).unwrap().unwrap();
        assert_eq!(block_by_hash.hash, genesis_hash);

        let block_by_num = server.get_block_by_number(0).unwrap().unwrap();
    ...
    async fn test_rpc_account_methods() {
        ...
        assert_eq!(server.get_balance(addr.to_string()).unwrap(), 1_000_000_000);
        assert_eq!(server.get_nonce(addr.to_string()).unwrap(), 0);
    }
    ...
    async fn test_rpc_transaction_methods() {
        ...
        let retrieved_tx = server.get_transaction_by_hash(tx_hash.clone()).unwrap().unwrap();
    ...
    async fn test_rpc_network_methods() {
        ...
        assert_eq!(server.net_peer_count().unwrap(), 0);
    ...
    async fn test_rpc_fee_methods() {
        let (server, _) = setup();
        assert_eq!(server.gas_price().unwrap(), 1);
        let tx = Transaction::new("a".into(), "b".into(), 100, vec![]);
        assert_eq!(server.estimate_gas(tx).unwrap(), 21000);
    }
}
